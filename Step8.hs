...
import System.Random.MWC (Gen, Variate, create, uniformR)
...
vmc ::
  (Fractional a, Ord a, PrimMonad m, Unbox a, Variate a) =>
  Gen (PrimState m) -> WorldSpec a -> World a -> m (World a)
vmc g WorldSpec
  {  dimension = d,
  ...
  let  vmc'
         ...
         do  !x <- replicateM d $ uniformR (-1, 1) g
